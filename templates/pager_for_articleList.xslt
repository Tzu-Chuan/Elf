    ...
    <nav class="text-center">
      ...
      <ul class="pagination pagination-lg">
        ...
        <xsl:if test="$pagerCurrentPageIndex+(-1) &gt;= 1">
          <li>
            <a href="javascript:doDetailSearchByPage({$pagerCurrentPageIndex+(-1)}, '{$orderField}', '{$typeId}', '{$typeGuid}')" aria-label="Next">
  ...
  <xsl:template name="pagernum">
    ...
    <xsl:if test="$pageIndex&lt;=$pagerEndPage">
      <xsl:choose>
        ...
        <xsl:otherwise>
          <li>
            <a href="javascript:doDetailSearchByPage({$pageIndex}, '{$orderField}', '{$typeId}', '{$typeGuid}')">
              <xsl:value-of select="$pageIndex"/>
            </a>
          </li>
        </xsl:otherwise>
